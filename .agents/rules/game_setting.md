...
* Travel consumes **1 stamina per kilometer** (reduced to **0.5 stamina/km** if riding a rented horse).
...
* **Difficulty Scaling:** Monster difficulty scales exponentially with depth:
  `Monster Level = clamp(1, 20, Math.floor(Depth / 1.5) + 1)`
...
8. **A stable** (Rent horses to halve stamina travel costs on roads and paths).
